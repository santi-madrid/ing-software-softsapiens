...
void CharacterView::_bind_methods() {
  ClassDB::bind_method(D_METHOD("get_amplitude"),
                       &CharacterView::get_amplitude);
  ClassDB::bind_method(D_METHOD("set_amplitude", "p_amplitude"),
                       &CharacterView::set_amplitude);
...
void CharacterView::_physics_process(double p_delta) {
  ...
  if (!health_bar) {
    if (has_node(
            "/root/Main/UI/HealthBar")) { // Ajustá si tu jerarquía es distinta
      health_bar =
          get_node<godot::TextureProgressBar>("/root/Main/UI/HealthBar");
  ...
  if (input->is_action_just_pressed("ui_cancel")) {
    Ref<PackedScene> pause_scene =
        ResourceLoader::get_singleton()->load("res://pause_menu.tscn");
  ...
  if (velocity.x != 0) {
    ...
    if (!sprite->is_playing() ||
        sprite->get_animation() != StringName("idle")) {
      sprite->play("idle");
    }
  }
...
double CharacterView::get_amplitude() const { return amplitude; }
...
bool CharacterView::take_damage(int amount) {
  ...
  bool result = presenter->take_damage(
      amount); // se ejecuta take_damage del modelo y actualiza la vida por el
               // danio recibido. true si la vida es 0 o menos
